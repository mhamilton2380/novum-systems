...
function GlowViz() {
  ...
  const draw = useCallback(() => {
    ...
    for (let x = 0; x < W; x += 28) for (let y = 0; y < H; y += 28) { ctx.beginPath(); ctx.arc(x, y, 1, 0, Math.PI * 2); ctx.fill(); }
    ...
    // Nodes: [label, xFrac, yFrac]
    const nodes: [string, number, number][] = [
      ["Accounting",   0.07, 0.24],
      ["Reporting",    0.07, 0.40],
      ["Scheduling",   0.07, 0.56],
      ["Workflows",    0.07, 0.72],
      ["Tasks",        0.93, 0.24],
      ["Documents",    0.93, 0.40],
      ["Field Ops",    0.93, 0.56],
      ["Integrations", 0.93, 0.72],
    ];

    nodes.forEach(([label, xf, yf], i) => {
      const nx = xf * W;
      const ny = yf * H;
      const phase = (t * 0.55 + i * 0.28) % 1;
      const segLen = 0.14;
      const p0 = Math.max(0, phase - segLen);
      ...
      ctx.strokeStyle = "rgba(100,130,200,0.13)"; ctx.lineWidth = 1; ctx.stroke();

      // Glow streak
      const brightness = Math.sin(phase * Math.PI) * 0.9 + 0.1;
      const grad = ctx.createLinearGradient(sx, sy, px, py);
      grad.addColorStop(0, "rgba(100,160,255,0)");
      grad.addColorStop(0.4, `rgba(130,180,255,${brightness * 0.7})`);
      grad.addColorStop(1, `rgba(180,215,255,${brightness})`);
      ctx.beginPath(); ctx.moveTo(sx, sy); ctx.lineTo(px, py);
      ctx.strokeStyle = grad; ctx.lineWidth = 2; ctx.stroke();

      // Glow dot at head
      const glow = ctx.createRadialGradient(px, py, 0, px, py, 8);
      glow.addColorStop(0, `rgba(160,205,255,${brightness * 0.95})`);
      glow.addColorStop(0.5, `rgba(100,155,240,${brightness * 0.4})`);
      glow.addColorStop(1, "rgba(100,155,240,0)");
      ctx.beginPath(); ctx.arc(px, py, 8, 0, Math.PI * 2);
      ctx.fillStyle = glow; ctx.fill();

      // Node pill
      const isActive = phase > 0.86 || phase < 0.1;
      const alpha = isActive ? 0.9 : 0.52;
      ctx.font = `${isActive ? 600 : 400} 11px 'DM Sans', sans-serif`;
      ctx.textBaseline = "middle";
      const tw = ctx.measureText(label).width;
      const pw = tw + 22; const ph = 26;
      const px2 = xf < 0.5 ? nx : nx - pw;
      const py2 = ny - ph / 2;

      // pill background
      ctx.fillStyle = isActive ? `rgba(58,85,133,${alpha * 0.38})` : `rgba(255,255,255,${alpha * 0.065})`;
      ctx.strokeStyle = isActive ? `rgba(110,155,245,${alpha * 0.65})` : `rgba(255,255,255,${alpha * 0.16})`;
      ctx.lineWidth = isActive ? 1.2 : 0.8;
      pillRect(ctx, px2, py2, pw, ph, 7); ctx.fill(); ctx.stroke();

      // dot
      ctx.fillStyle = isActive ? `rgba(165,205,255,${alpha})` : `rgba(255,255,255,${alpha * 0.45})`;
      ctx.beginPath(); ctx.arc(px2 + 10, ny, 2.8, 0, Math.PI * 2); ctx.fill();

      // label
      ctx.fillStyle = isActive ? `rgba(195,220,255,${alpha})` : `rgba(255,255,255,${alpha * 0.62})`;
      ctx.fillText(label, px2 + 18, ny);
    });

    // Center glow
    const cg = ctx.createRadialGradient(cx, cy, 0, cx, cy, 55);
    cg.addColorStop(0, "rgba(80,110,200,0.2)"); cg.addColorStop(1, "rgba(80,110,200,0)");
...
function Phone() {
  ...
  return (
    <div style={{
      ...
      <div style={{ background: "#0C0E13", border: "1.5px solid rgba(255,255,255,0.13)", borderRadius: "26px", padding: "11px 8px 15px" }}>
...
export default function HomePage() {
  useScrollReveal();
  return (
    <div style={{ background: "var(--white)", color: "var(--text)", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ padding: "20px 24px 0" }}>
        <div style={{
          ...
          {/* Left — copy */}
          <div style={{ position: "relative", zIndex: 4, padding: "72px 0 72px 56px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            ...
            <h1 className="font-serif" style={{ fontSize: "clamp(2.6rem, 3.8vw, 4.4rem)", lineHeight: 1.04, letterSpacing: "-0.025em", color: "#fff", marginBottom: 24 }}>
              Software that{" "}
        ...
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "80px", alignItems: "start" }} className="problem-grid">
            <div className="fade-section">
              <div className="pill-tag" style={{ border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text-soft)", marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }} className="fade-section">
              {[
                ...
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "20px 24px", background: "var(--red-bg)", border: "1px solid var(--red-border)", borderRadius: 12, transition: "transform 0.25s, box-shadow 0.25s" }}
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }} className="solution-grid">
              {[
                ...
                {
                  icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="rgba(160,185,255,0.75)" strokeWidth="1.5" /><path d="M12 7v5l3 3" stroke="rgba(160,185,255,0.75)" strokeWidth="1.5" strokeLinecap="round" /></svg>,
                ...
                <div key={i} style={{ padding: 32, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, transition: "border-color 0.25s, background 0.25s, transform 0.25s" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(100,145,240,0.4)"; (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.07)"; (e.currentTarget as HTMLElement).style.transform = "translateY(-3px)"; }}
                ...
                >
                  <div style={{ width: 48, height: 48, background: "rgba(255,255,255,0.07)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24 }}>{item.icon}</div>
      ...
      <section style={{ padding: "100px 48px" }}>
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 56, flexWrap: "wrap", gap: 24 }} className="fade-section">
            <div>
              <div className="pill-tag" style={{ border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text-soft)", marginBottom: 18 }}>
                <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--text-soft)", display: "inline-block" }} />
                Our Systems
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              { name: "Forge", badge: "Fully Custom", bc: "#6D4FBB", bb: "#F3F0FC", bbd: "#D9D0F5", desc: "For operations that don't fit any mold. Forge is a completely custom-built system designed from the ground up around your unique structure, terminology, and workflows. No templates, no constraints — just your system, architected exactly the way your business runs.", features: ["Full operational architecture from scratch", "Custom data model & integrations", "Proprietary workflow engine", "White-glove build & deployment"] },
              { name: "OpsCore", badge: "Operations Hub", bc: "var(--accent)", bb: "var(--accent-bg)", bbd: "var(--accent-border)", desc: "Your entire operation unified in one intelligent command center. OpsCore uses AI to generate tasks from emails, texts, and documents automatically. Your team communicates via built-in chat, shares notes, tracks projects, and monitors budgets — all in one place. And it connects to the tools you already use: accounting platforms like QuickBooks, CRMs like Salesforce or HubSpot, project tools like Monday or Asana, communication tools like Gmail or Outlook, ERP systems, and more. One platform. Every tool. Zero switching.", features: ["AI task generation from email/text/PDF", "Built-in team chat & shared notes", "Project-filtered calendar & tasks", "QuickBooks, Salesforce, HubSpot & more", "Custom role dashboards", "Workflow automation"] },
              { name: "FieldOps", badge: "Field Service", bc: "#236B4E", bb: "#EEF7F3", bbd: "#C4E0D5", desc: "End-to-end field service management built around your crews, territories, and job types. From the moment a job is booked to the final invoice — scheduling, dispatch, field access, photo documentation, and automated billing all run through one connected system.", features: ["Visual scheduling & dispatch", "Mobile field access & job tracking", "Automated invoicing on completion", "Customer records & property history"] },
              { name: "ProjectOps", badge: "Project-Based", bc: "#2C4E8A", bb: "#EEF2FA", bbd: "#C0CEEB", desc: "Full project lifecycle management from bid to close. Track live budget vs. actuals, manage vendors and subs, monitor milestone completion, and report on profitability across every active project — all in one system built around your workflow.", features: ["Live budget-vs-actual tracking", "Vendor & sub management", "Milestone & Gantt-style timelines", "Document & RFI management", "Profitability reporting"] },
            ].map((sys, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "240px 1fr auto", gap: 48, alignItems: "center", padding: "36px 40px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, transition: "border-color 0.2s, box-shadow 0.2s, transform 0.25s", cursor: "pointer" }}
                className="sys-row fade-section"
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--border-strong)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 6px 28px rgba(0,0,0,0.07)"; (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; }}
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {sys.features.map(f => <span key={f} style={{ padding: "3px 12px", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 6, fontSize: "0.77rem", color: "var(--text-soft)" }}>{f}</span>)}
                  </div>
                </div>
      ...
      <section style={{ padding: "100px 48px", background: "var(--surface)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 72 }} className="fade-section">
            <div className="pill-tag" style={{ border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text-soft)", marginBottom: 20 }}>
              <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--text-soft)", display: "inline-block" }} />
              How It Works
            </div>
            <h2 className="font-serif" style={{ fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", lineHeight: "1.1", letterSpacing: "-0.02em" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, position: "relative" }} className="process-grid">
            ...
            {[
              ...
              <div key={i} style={{ textAlign: "center", padding: "0 20px" }} className="fade-section">
                <div style={{ width: 72, height: 72, borderRadius: "50%", background: "var(--bg)", border: "1.5px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 32px", position: "relative", zIndex: 1, transition: "border-color 0.25s, transform 0.25s" }}
